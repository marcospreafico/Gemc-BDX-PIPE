...
double MeV2peOV[8] = { 38.4615, 33.3333, 37, 40, 33.8983, 42.5532, 40, 41.6667 }; //check all parameters
  ...
  int ADC12 =0;
	...
	if (veto_id == 5) {
		...
		if (Etot > 0) {
			...
			peL = G4Poisson(etotL * light_yield * sensor_qe)*1000;
			peR = G4Poisson(etotR * light_yield * sensor_qe)*1000;
					...
					for (auto ghit:ghits) {
						...
						for (unsigned int j = 0; j < 8; j++) {                       //looping on 8 sipm
							if (chan == 701) { //OV                      // outer veto sipm staggered by 22.5deg
								...
								att_z = att_z_IV;
								DeltaZ=zCluster+aHit->GetDetector().dimensions[2];//9
							}
...
//starting paddles
		...
		double optical_coupling[3] = {1., 1., 1.};
//		double optical_coupling[3] = {0., 1., 0.37};
		...
		sensor_surface = 3*3; // 1" pmt R-> (2.5/2 TBC)   
		sensor_effective_area = 1.;
...
/*	if (channel == 14)        // TOp read by Left and Right sides
			{
		double pd_len = 310 / 10.; //in cm
		double y = pd_len / 2 + zz / 10;			//in cm
		double normfactor[2] = { 1.0, 1.0 };
