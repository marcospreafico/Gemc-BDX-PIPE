...
G4MuonScalarProduction::G4MuonScalarProduction(const G4String& processName, G4ProcessType type) :
		G4VDiscreteProcess(processName, type), MParticle(G4ParticleSkeleton::Particle()->GetPDGMass()), LowestEnergyLimit(MParticle), //the lower limit of the minimum energy
		HighestEnergyLimit(1e21 * eV), // ok to 1e21eV=1e12GeV, then LPM suppression
		CrossSecFactor(1.), fData(0), gXsecData(0), hXsecDistr(0) {
	SetProcessSubType(G4ProcessSkeletonProcessID);
	MeanFreePath = DBL_MAX;

	/*The data is loaded "on the fly", in the GetMeanFreePath - for the given ELEMENT.*/
	char* pPath;
	pPath = getenv("CROSS_SECTION_PATH");
	if (pPath == NULL) {
		std::cout << std::endl << std::endl;
		std::cout << "Environmental variable for database CROSS_SECTION_PATH is not set." << std::endl;
		...
		std::cout << std::endl << std::endl;
		if (messageOut) {
			std::cout << "G4ProcessSkeleton creator, the environmental variable CROSS_SECTION_PATH is set to: " << dataPathFolder << std::endl;
			messageOut = false;
		}
		dataPathFolder += "/mother"; // in the original scalar process here I was taking the /muon folder --> mother particles 2
		struct stat sb;
		if (!(stat(dataPathFolder.c_str(), &sb) == 0 && S_ISDIR(sb.st_mode))) {
			std::cout << std::endl << std::endl;
			std::cout << "G4ParticleSkeleton, mother particle folder is missing within CROSS_SECTION_PATH" << std::endl;
			exit(1);
		}
...
G4bool G4MuonScalarProduction::IsApplicable(const G4ParticleDefinition& particle) {
    // here I need to change which is the particle for which the process is available
