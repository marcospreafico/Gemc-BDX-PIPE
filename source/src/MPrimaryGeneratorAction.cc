...
MPrimaryGeneratorAction::MPrimaryGeneratorAction(goptions *opts, MEventAction *evtAction) {
...
void MPrimaryGeneratorAction::GeneratePrimaries(G4Event *anEvent) {
    ...
    /*A.C. these were moved here from MEventAction::BeginOfEventAction since this is called before*/
    if (evtAction->do_RETRIEVE_RANDOM) {
        evtAction->retrieveRandom();
    }

    if (evtAction->do_SAVE_RANDOM) {
        evtAction->saveRandom();
    }
